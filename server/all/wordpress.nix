{
  ...
  infra = {
    ...
    wordpress = {
      id = 150;
      name = "wordpress";
      hostname = infra.wordpress.name;
      domain = infra.domain.user;
      fqdn = "${infra.wordpress.hostname}.${infra.wordpress.domain}";
      ip = "${infra.net.user}.${toString infra.wordpress.id}";
      network = infra.cidr.user;
      namespace = infra.namespace.user;
      localbind = {
        ip = infra.localhost.ip;
        port.http = infra.localhost.port.offset.http + infra.wordpress.id;
      };
    };
  };
in {
  ...
  #-=# SYSTEMD #=-#
  #################
  systemd.network.networks.${infra.wordpress.namespace}.addresses = [
    {Address = "${infra.wordpress.ip}/32";}
  ];
  ...
  ####################
  networking = {
    extraHosts = "${infra.wordpress.ip} ${infra.wordpress.hostname} ${infra.wordpress.fqdn}";
  ...
  services = {
    wordpress.sites."localhost" = {
      ...
      settings.WPLANG = "de_DE";
      virtualHost.listen = [
        {
          ip = infra.wordpress.localbind.ip;
          port = infra.wordpress.localbind.port.http;
          ssl = false;
        }
      ];
    };
    caddy = {
      enable = false;
      virtualHosts."${infra.wordpress.fqdn}".extraConfig = ''
        bind ${infra.wordpress.ip}
        reverse_proxy ${infra.wordpress.localbind.ip}:${toString infra.wordpress.localbind.port.http}
        tls ${infra.pki.acmeContact} {
              ca ${infra.pki.url}
              ca_root ${infra.pki.caFile}
        }
        @not_intranet {
          not remote_ip ${infra.wordpress.network}
        }
        respond @not_intranet 403
        log {
          output file ${config.services.caddy.logDir}/${infra.wordpress.name}.log
        }'';
    };
  };
}
