...
in {
  ...
  ####################
  networking = {
    extraHosts = "${infra.webpki.ip} ${infra.webpki.hostname} ${infra.webpki.fqdn}.";
    firewall.allowedTCPPorts = infra.webpki.port;
  };
  ...
  services = {
    caddy = {
      enable = true;
      virtualHosts = {
        "${infra.webpki.fqdn}".extraConfig = ''
          bind ${infra.webpki.ip}
          ...
          @not_intranet {
            not remote_ip ${infra.webpki.access.cidr}
          }
          respond @not_intranet 403
          log {
            output file ${config.services.caddy.logDir}/access/${infra.webpki.name}.log
          }'';
      };
    };
  };
